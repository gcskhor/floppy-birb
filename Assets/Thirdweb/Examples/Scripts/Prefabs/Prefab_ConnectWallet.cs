...
public class Prefab_ConnectWallet : MonoBehaviour
{
    ...
    public async void OnConnect(WalletProvider wallet, string password = null, string email = "joe@biden.com", WalletProvider personalWallet = WalletProvider.LocalWallet)
    {
        try
        {
            ...
            StartScreen.GetComponent<StartScreenScript>().toggleStartScreen(ConnectedState, DisconnectedState, address);

            // ADD GAMEMANAGER CONNECTION LOGIC HERE
            GameManager.instance.UpdateGameState(GameState.Connected);
        }
        catch (Exception e)
        {
            OnDisconnect();
    ...
    public async void OnDisconnect()
    {
        try
        {
            ...
            // ADD GAMEMANAGER CONNECTION LOGIC HERE
            StartScreen.GetComponent<StartScreenScript>().untoggleStartScreen(ConnectedState, DisconnectedState);
            GameManager.instance.WalletDisconnect();
        }
        catch (Exception e)
        {
